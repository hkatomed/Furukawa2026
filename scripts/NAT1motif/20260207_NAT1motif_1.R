...
writeXStringSet(utr5_seqs, filepath = "20251015_MANE_5utr.fasta")
write.table(utr5_df, file = "20251015_MANE_5utr.txt", quote = FALSE, col.names = TRUE, row.names = FALSE, sep = "\t")
library(openxlsx)
write.xlsx(utr5_df, file = "20251015_MANE_5utr.xlsx")
...
setwd(HOMEDIR)
setwd("20251015_MANE_5utr")
write.xlsx(NAT1motifDF, file = "20251015_MANE_5utr_NAT1motif.xlsx")
genome[["chr11"]][10807293:10807295]
utr5_MANEtagList <- gff_utr5@elementMetadata@listData$tag
utr5_MANEtag <- character(length = length(utr5_MANEtagList))
for(i in 1:length(utr5_MANEtagList)){
	utr5_MANEtag[i] <- paste(utr5_MANEtagList[[i]], collapse = ",", sep = "")
...
setwd(HOMEDIR)
setwd("20251015_MANE_5utr")
write.xlsx(nonAUGgenes, file = "20251015_MANE_nonAUGgenes.xlsx")
