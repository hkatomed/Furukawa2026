...
utr3_seqs <- readDNAStringSet(filepath = "20251016_MANE_3utr.fasta")
NAT1motif <- "GCCGCCRNNNNNG"
NAT1motif <- as.character(reverseComplement(DNAString(NAT1motif)))
NAT1motifDF_Rv <- data.frame(gene_name = character(length = 0), utr3_len = integer(length = 0), 
			start = integer(length = 0), seq = character(length = 0), 
			RNN = character(length = 0), NNN = character(length = 0))
for(i in 1:length(utr3_seqs)){
	geneName <- names(utr3_seqs)[i]
...
setwd(HOMEDIR)
setwd("20251016_MANE_3utr")
write.xlsx(NAT1motifDF_Rv, file = "20251016_MANE_3utr_NAT1motif_Rv.xlsx")
